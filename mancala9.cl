...
;;;
;;; MANCALA - v.9.0 P2P   A. de Salabert 2016
;;; Implementa una version del juego Mancala con las variantes expuestas en la P3 de 2016
;;;
...
(defvar *vers*           "9.0" ) ; Software version "x.y" = mancala X, rel. Y
          ...
          (let ((total-contr (suma-fila tablero (lado-contrario lado))))
            (cond                                                 ; Hay entrega o captura?
             ...
             (t (anade-ficha tablero lado desde)
              ...
              nil))))))
       ((and es-su-zona *kalaha* (> tengo 2) (= captura 0))       ; Entrega especifica Mancala9: propias>2 y contrario=0
        ;;(format t "~%Entrega especifica M9: tengo>2 (~A) y contrario=0 (~A)" tengo captura)
        (put-fichas tablero lado desde 0)                         ; Entrega a contrario y termina
        (put-fichas tablero (lado-contrario lado) hoyo-contrario (+ captura 1 tengo))
        (show (format nil " => Entrega ~D+1 al contrario y cede turno." tengo))
        nil)
       ((and (not es-su-zona) *kalaha* (> tengo 3) (= captura 0)) ; Robo especifico Mancala9: termina en contrario>2 y propias=0
        ;;(format t "~%Robo especifico M9: sembrando en contrario tengo=0 (~A) y contrario>2 (~A)" captura tengo)
...
(defun minimax-a-b (estado profundidad-max f-eval)
  (let* ((oldverb *verb*)  (*verb* nil)
         (estado2 (minimax-a-b-aux estado profundidad-max  -999999 999999 t profundidad-max f-eval 0))
         (*verb* oldverb))
    estado2))
...
;;; ------------------------------------------------------------------------------------------
;;; Funcion auxiliar minimax-a-b-aux
;;; RECIBE:   estado, profundidad-actual, alfa, beta,
;;;           devolver-movimiento: flag que indica si devolver un estado (llamada raiz) o un valor numerico (resto de llamadas)
;;;           profundidad-max    : limite de profundidad
;;;           f-eval             : funcion de evaluacion
;;;           alfa             : param. alfa de minimax
;;;           beta             : param. beta de minimax
;;;           estado-actual    : flag que informa si estamos en MAX(0) o MIN(1)
;;; DEVUELVE: valor minimax en todos los niveles de profundidad, excepto en el nivel 0 que devuelve el estado del juego tras el
;;;           movimiento que escoge realizar
;;; ------------------------------------------------------------------------------------------
(defun minimax-a-b-aux (estado profundidad-actual alfa beta devolver-movimiento profundidad-max f-eval estado-actual)
  (cond 
    ((= profundidad-actual 0) 
        (if ( = (mod profundidad-max 2) 0)
          (unless devolver-movimiento  (funcall f-eval estado))
          (unless devolver-movimiento  (- (funcall f-eval estado)))))
    (t 
      (let* (
        ...
        (cond 
          ((null sucesores) 
            (if ( = (mod profundidad-actual 2) 0)
              (unless devolver-movimiento  (funcall f-eval estado))
              (unless devolver-movimiento  (- (funcall f-eval estado)))))
          ((= estado-actual 0)
              (loop for sucesor in sucesores until (>=  alfa  beta) do
                (let* (
                  (result (minimax-a-b-aux sucesor (- profundidad-actual 1) alfa beta nil profundidad-max f-eval 1)))
                  (cond 
                    ((<= alfa result) 
                      (setq alfa result)
                      (setq mejor-sucesor sucesor))
          ...
          ((= estado-actual 1)
            (loop for sucesor in sucesores until (>= alfa beta) do
                (let* (
                  (result (minimax-a-b-aux sucesor (- profundidad-actual 1) alfa beta nil profundidad-max f-eval 0)))
                  (cond 
                    ((>= beta result)
                      (setq beta result)
                      (setq mejor-sucesor sucesor))
                  ((>=  alfa  beta)                    
                    (if  devolver-movimiento mejor-sucesor beta)))))
              (if  devolver-movimiento mejor-sucesor beta)))))))
...
;;; f-juego que utiliza busqueda minimax pero evalua aleatoriamente
;;; ------------------------------------------------------------------------------------------
...
(defun f-eval-Bueno-R (estado)
  (if (juego-terminado-p estado)   ; Condicion especial de juego terminado
      (* 50 (- (suma-fila (estado-tablero estado) (estado-lado-sgte-jugador estado))
...
(defvar *jdr-mmx-Bueno-R*)
(setf *jdr-mmx-Bueno-R* (make-jugador
                        :nombre   '|Ju-Mmx-Bueno-R|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-Bueno-R))
...
(defun mi-f-ev (estado)
  (- (- (get-pts (estado-lado-sgte-jugador estado))                                                 
        (+ (get-fichas (estado-tablero estado)(estado-lado-sgte-jugador estado) 6)   
           ...
           (get-fichas (estado-tablero estado)(estado-lado-sgte-jugador estado) 1)))   
     ...
     (- (get-pts (lado-contrario (estado-lado-sgte-jugador estado)))                                 
        (+ (get-fichas (estado-tablero estado)(lado-contrario (estado-lado-sgte-jugador estado)) 6)
           ...
           (get-fichas (estado-tablero estado)(lado-contrario (estado-lado-sgte-jugador estado)) 4)
           (get-fichas (estado-tablero estado)(lado-contrario (estado-lado-sgte-jugador estado)) 3)
           (get-fichas (estado-tablero estado)(lado-contrario (estado-lado-sgte-jugador estado)) 2)
           (get-fichas (estado-tablero estado)(lado-contrario (estado-lado-sgte-jugador estado)) 1)))))
...
;;; *debug-mmx* activa *verb* tambien para jugadores automaticos (normalmente desactivado).
;;;(setq *verb* t)
;;;(setq *debug-level* 2)
;;;(setq *debug-mmx* nil)
;;;(setq *verjugada*   t)
...
;;; Se fuerza la posicion explicada en el punto 5 (pag.5) del enunciado.
...
; PRUEBAS CON NUESTRO JUGADOR
;----------------------------------------------------------------------------
(partida 0 2 (list *mi-jugador*      *jdr-mmx-Bueno*))
;;;(partida 0 2 (list *mi-jugador*      *jdr-mmx-bueno*))
;;;(partida 0 2 (list *mi-jugador*      *jdr-mmx-Regular*))
;;;(partida 0 2 (list *mi-jugador*      *jdr-mmx-regular*))
